...
##3
##Older/deprecated but good for threadpooling
...
for i in comps:
    ...
    for ae1 in aes:
        a,b,c,d=0,0,0,0
        print(ae1)
        c=len(base_df[(base_df['0']==base_drug) & (base_df['pt']==ae1)])
        d=len(base_df[(base_df['0']==base_drug) & (base_df['pt']!=ae1)])
        a+=sum([len(base_df[(base_df['0']==cdrug) & (base_df['pt']==ae1)]) for cdrug in comps[i]])
        b+=sum([len(base_df[(base_df['0']==cdrug) & (base_df['pt']!=ae1)]) for cdrug in comps[i]])
...
comp_df.to_csv('disproportionailty_comps2drugs.csv')
